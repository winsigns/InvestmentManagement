...
import com.winsigns.investment.framework.measure.data.MeasureDataJsonSerde;
import com.winsigns.investment.framework.measure.data.MeasureDataJsonSerializer;

public class KafkaStreamTopoBuilder {

  public KafkaStreamTopoBuilder(KafkaConfiguration kafkaConfiguration) {
    appId = kafkaConfiguration.getAppId();
    brokerHost = kafkaConfiguration.getBrokerHost();
    zookeeperHost = kafkaConfiguration.getZookeeperHost();
  }
  ...
  public void start(MeasureManager measureManager) {
    ...
    for (IMeasure measure : measureManager.getMeasures()) {
      ...
      log.info(String.format("addProcessor: name:%s, deps:%s", processorName,
          String.join(" / ", measure.getRealDependencies())));
      builder = builder.addProcessor(processorName, () -> new MeasureProcessor(measure),
          measure.getRealDependencies());
      log.info(String.format("addStateStore: name:%s, deps:%s", stateStoreName, processorName));
      builder = builder.addStateStore(Stores.create(stateStoreName).withKeys(Serdes.String())
          .withValues(jsonSerde).inMemory().build(), processorName);
      log.info(String.format("addSink: name:%s, deps:%s", sinkName, processorName));
      builder = builder.addSink(sinkName, measure.getName(), stringSerializer, jsonSerializer,
          processorName);
    }
