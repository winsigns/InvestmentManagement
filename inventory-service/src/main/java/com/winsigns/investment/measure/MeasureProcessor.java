...
public class MeasureProcessor extends AbstractProcessor<String, MeasureData>{
	...
	public void init(ProcessorContext context) {
		super.init(context);
		kvStore = (KeyValueStore<String, MeasureData>) context.getStateStore(String.format("%s.%s.store", measure.getHost(), measure.getName()));
	}
	...
	public void process(String key, MeasureData value) {
		...
		if(key.endsWith(".internal")){
			if (measure.needToProcessInternal()){
				//内部指标变化，不管版本，优先处理
				MeasureData result = measure.calc(key, value, true, kvStore.get(measure.getKey("internal")));
		...
		if (key.equals("timer.sequence")){
			...
			log.info(String.format("measure=%s.%s, time.seq recved, currVersion=%s", measure.getHost(), measure.getName(), currVersion));
		}

		MeasureData readyToCalc = kvStore.get(String.format("readyToCalc.%s", currVersion));
		log.info(String.format("measure=%s.%s, readyToCalc=%s, currVersion=%s", measure.getHost(), measure.getName(), (readyToCalc != null && readyToCalc.value > 0) ? "true" : "false", currVersion));
