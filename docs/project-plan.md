...
gateway部分的工作主要分成以下几个部分：请求转发、额外查询请求、针对界面功能的权限处理和针对界面的其他接口。请求转发部分工作量可以忽略不记。额外查询请求在第1阶段应该跑通。在第2阶段，主要是工作量的问题。现在按每个页面的查询接口平均2人天计算，一共需要80人天。针对界面功能的权限处理目前因为权限服务还没有设计和开发，无法提前打通路径，需要在第2阶段进行设计和开发。现在暂时预估详细设计工作量10人天，开发工作量30人天。针对界面的其他接口目前还不能确定有多少。暂时预留30人天的余量。gateway部分的工作大致需要 80+40+30=150人天。
